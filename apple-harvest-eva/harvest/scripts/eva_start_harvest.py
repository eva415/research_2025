...
# Add import for my FlexToFListener controller
import sys
from pathlib import Path
# Add the controller folder to the path
controller_path = Path.home() / "git/research_2025/apple-harvest-eva/harvest_control/scripts"
sys.path.append(str(controller_path))
import eva_controller
...
class StartHarvest(Node):
    def __init__(self):
        ...
        self.declare_parameter('enable_pressure_servo', False) # I don't use pressure servoing
    ...
    def start_flex_tof_controller(self):
        self.get_logger().info("Starting FlexToFListener controller...")
        # Create node
        self.controller_node = eva_controller.FlexToFListener()
        # Use MultiThreadedExecutor for concurrency
        self.controller_executor = rclpy.executors.MultiThreadedExecutor()
        self.controller_executor.add_node(self.controller_node)
        
        # Spin in a separate thread if you want your main harvest loop to continue
        import threading
        self.controller_thread = threading.Thread(target=self.controller_executor.spin, daemon=True)
    ...
    def start(self): 
        ...
        for idx, coord in enumerate(apple_poses.poses):
            ...
            # Stage 4: tactile servo, grasp, and pick
            # Instead of pressure servo / pick_controller, run your controller
            input('Done with approach, hit enter to start FlexToF controller')
            self.start_flex_tof_controller()

            # Optionally wait for some condition before continuing
            input('Press enter to continue to next apple')
